...
namespace Plugins.Mesh.OBJ
{
    public class OBJLoader
    {
        ...
        private static readonly Dictionary<string, Action<string>> m_ParseOBJActionDictionary = new Dictionary<string, Action<string>>
        {
            ...
            { "v", (lEntry) => { m_OBJData.m_Vertices.Add(Extension.Utils.ParseVector3String(lEntry)); } },
            { "vn", (lEntry) => { m_OBJData.m_Normals.Add(Extension.Utils.ParseVector3String(lEntry)); } },
            { "vt", (lEntry) => { m_OBJData.m_UVs.Add(Extension.Utils.ParseVector2String(lEntry)); } },
            { "vt2", (lEntry) => { m_OBJData.m_UV2s.Add(Extension.Utils.ParseVector2String(lEntry)); } },
            { "vc", (lEntry) => { m_OBJData.m_Colors.Add(Extension.Utils.ParseVector4String(lEntry).ToColor()); } },
            { "f", PushOBJFace },
            { "g", PushOBJGroup },
        };
        ...
        private static readonly Dictionary<string, Action<string>> m_ParseMTLActionDictionary = new Dictionary<string, Action<string>>
        {
            ...
            { "Ka", (lEntry) => { m_CurrentMaterial.m_AmbientColor = Extension.Utils.ParseVector3String(lEntry).ToColor(); } },
            { "Kd", (lEntry) => { m_CurrentMaterial.m_DiffuseColor = Extension.Utils.ParseVector3String(lEntry).ToColor(); } },
            { "Ks", (lEntry) => { m_CurrentMaterial.m_SpecularColor = Extension.Utils.ParseVector3String(lEntry).ToColor(); } },
